...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory {
    ...
    //以一个Key将Fragment的mIndex保存到bundle中去
    @Override
    ...
    //同样：以一个key获取bundle中的Fragment的Index，并从mActivie中获取到这个Fragment
    @Override
        ...
        if (f.mState < newState) {
            ...
            switch (f.mState) {
                case Fragment.INITIALIZING:
                    ...
                    f.mRetaining = false;
                    if (f.mFromLayout) {//这边执行的条件是Layout文件创建的Fragment
                ...
                case Fragment.CREATED:
                    if (newState > Fragment.CREATED) {
                        ...
                        if (!f.mFromLayout) {
                            ...
                            if (f.mContainerId != 0) {
                                /** 一般情况下其实就是找到了自身，但从这边看，其实可以设置为一个子View */
                            ...
                            if (f.mView != null) {
                                ...
                                if (container != null) {
                                    ...
                                    /**
                                     * 这里其实可以看出来，Fragment只不过是维护某一块View和Activity之间的
                                     * 生命周期，View实际还是以普通的子View身份加入进去的
                                     * */
                ...
                case Fragment.ACTIVITY_CREATED://created和stoped之后，更高阶的state则表示start了
                case Fragment.STOPPED:
                    if (newState > Fragment.STOPPED) {
                        if (DEBUG) Log.v(TAG, "moveto STARTED: " + f);
                        f.performStart();
                    }
                case Fragment.STARTED://start之后，更高阶的state则表示start了
    ...
    void makeActive(Fragment f) {
        if (f.mIndex >= 0) {//表明已经被添加过了
            return;
        }

        /**
         * 这边的整体数据结构是这样的，涉及到三个List变量：mActive，mAdded和mAvailIndices。
         * 一个Activity实例化FragmentManager之后，实例化添加的每一个Fragment都有一个下标，即mIndex。
         * 添加的每一个Fragment都会被记录到mActive中去，即调用makeActive方法，在这里就要设置mIndex。
         * 那么怎么设置呢？
         * 添加到mActive的Fragment不会被移除（这里的移除是指调用ArrayList的remove方法），而只会设置为null。
         * 即mActive的大小只会增加不会减少。因此就产生了下面的If...Else。如果直接将mActive.size()设置为
         * Fragment的mIndex，等于从mActive中分配新的下标资源。而因为makeInactive会将mActive中的一些位置
         * 置为null，因此这些位置就会空出来，此时Google就做了一件事：将这些位置记录在mAvailIndices中，表明
         * 这些位置仍然可用，当我们要添加一个Fragment并分配mIndex的时候，就优先从这些可分配的回收下标资源中去获取
         * 非常节省：虽然即使不采用这样的方法，一个Activity也完全不太可能耗尽Integer范围内的下标，但思想不错。
         * */
        if (mAvailIndices == null || mAvailIndices.size() <= 0) {
            if (mActive == null) {
    ...
    /**
     * 在FragmentActivity中，这个返回值会被onSaveInstanceState的bundle写入保存下来
     */
    Parcelable saveAllState() {
        ...
        /** 遍历所有的添加过的Fragments */
        int N = mActive.size();
        FragmentState[] active = new FragmentState[N];//每一个Fragment的状态都对应到一个FragmentState中
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (f != null) {
                ...
                //只要有一个不为Null，则表示有添加过Fragments
                haveFragments = true;

                /** 取出Fragment的状态，保存到fs中去：包含Fragment的整个儿环境：ContainerId, FragmentId,
                 * Tag等 */
                ...
                if (f.mState > Fragment.INITIALIZING && fs.mSavedFragmentState == null) {
                    ...
                    if (f.mTarget != null) {
                        ...
                        /** 将Target Fragment的Index保存到mSavedFragmentState里面去 */
                    ...
                    /** fs刚刚新建，mSavedFragmentState肯定为null
                     * 具体可以查看f.mState的定义，即什么时候状态是INITIALIZING */
        ...
        /**
         * mActive中的Fragment只有添加和设置为null的过程；
         * mAdded则又添加和删除；
         * 为什么要把Index记录下来？
         * */
        int[] added = null;
    ...
    void restoreAllState(Parcelable state, ArrayList<Fragment> nonConfig) {
        ...
        if (nonConfig != null) {
            for (int i = 0; i < nonConfig.size(); i++) {
                ...
                /** 重点哦！！注意，这边恢复的时候，Fragment是知道的mIndex
                 * 看原生注解，这个貌似和之前保存的没啥关系 */
        ...
        // their saved state.
        /**
         * 此处才是恢复之前保存的Fragment，并添加到mActivie中去
         * */
        ...
        /** 恢复mAdded
         * So：为什么需要保存一个int数组：最廉价的恢复mAdded的方式 */
        if (fms.mAdded != null) {
